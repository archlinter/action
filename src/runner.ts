  ...
  try {
    ...
    try {
      if (ref.includes('/')) {
        ...
        await exec.exec('git', ['fetch', '--no-tags', '--prune', '--depth=1', remote, `+refs/heads/${branch}:refs/remotes/${remote}/${branch}`], {
          cwd,
          ignoreReturnCode: true
        });
      } else {
        // Try to fetch from origin as a fallback
        await exec.exec('git', ['fetch', '--no-tags', '--prune', '--depth=1', 'origin', `+refs/heads/${ref}:refs/remotes/origin/${ref}`], {
          cwd,
          ignoreReturnCode: true
        });
      }
    } catch (fetchError) {
      core.warning(`Failed to fetch ref "${ref}": ${fetchError instanceof Error ? fetchError.message : String(fetchError)}`);
    }
  }
}
